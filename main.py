...
with st.sidebar.expander("🌍 Scenario Parco Auto", expanded=True):
    bev_base_2030 = st.number_input("Target BEV Palermo 2030 (Scenario Base)", value=5000)
    stress_bev = st.slider("Stress Test Adozione BEV (%)", 50, 150, 100) / 100
    public_share = st.slider("Quota Ricarica Pubblica (%)", 10, 80, 30, help="Percentuale di proprietari BEV senza ricarica privata.") / 100
...
with st.sidebar.expander("🔧 Scelte Tecniche"):
    ...
    kwh_per_sessione = st.number_input("kWh medi richiesti per ricarica", value=35)
...
# 1. Calcolo Funnel Mercato (Dati Palermo)
...
energia_kwh = auto_clienti_anno * 3000 # Consumo annuo stimato per auto basato su report

# 2. Capacità e Dimensionamento (Vincoli di tempo)
...
with c1:
    ...
    st.pyplot(fig1)
    st.markdown(r"""
    **Formula:** $N_{auto} = \frac{OPEX_{day} + Ammortamento_{day}}{kWh_{sess} \cdot (P_{vendita} - C_{energia})}$  
    *Spiegazione:* Indica il numero minimo di auto al giorno necessarie per coprire i costi operativi e il rientro del capitale investito (Ammortamento su 5 anni).
    """)
...
with c2:
    ...
    st.pyplot(fig2)
    st.markdown(r"""
    **Formula:** $CF_{cum, t} = \sum_{i=2026}^{t} (EBITDA_i - CAPEX_i)$  
    *Spiegazione:* Questo grafico mostra il momento esatto (Payback Period) in cui il progetto inizia a generare ricchezza netta dopo aver ripagato l'investimento iniziale.
    """)
...
with c3:
    ...
    *Spiegazione:* Il sistema calcola quante colonnine servono per non superare le ore operative impostate. Nella strategia Multisito, l'espansione avviene geograficamente.
    """)
...
with c4:
    ...
    st.pyplot(fig4)
    st.markdown(r"""
    **Formula:** $EBITDA = Ricavi - (Costi_{Var} + Costi_{Fissi})$  
    *Spiegazione:* Visualizza l'incidenza dei costi operativi (energia) e degli investimenti (CAPEX) rispetto alla capacità di generare utile lordo.
    """)
...
with st.expander("📚 Intelligence Report - Nota per il Management"):
    st.markdown(f"""
    - **Capacità Fisica**: Una {tecnologia} richiede circa **{kwh_per_sessione/potenza_kw*60:.0f} minuti** per erogare {kwh_per_sessione} kWh. 
    - **Saturazione**: Al 2030, intercetterai il **{quota_stazione[-1]*100:.1f}%** del mercato di Palermo con un flusso di **{auto_clienti_anno[-1]:,.0f} auto all'anno**.
    - **Pianificazione**: L'allocazione {allocazione} garantisce che la stazione non vada in overload superando le **{ore_max_giorno} ore/giorno** di utilizzo.
    """)
