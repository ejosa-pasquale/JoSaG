...
DATASET_NAME = "BEV Sicilia per provincia (stock) — input utente"
DATASET_VERSION = "2026-01-23 (tabella fornita in chat)"
...
st.caption(
    "Nota: il CAPEX è mostrato **anno per anno** (quanto investi in ciascun anno). "
    "Se una colonnina aggiuntiva entra in un anno specifico, il CAPEX si concentra lì."
)

# --- KPI principali (manager-friendly) ---
k1, k2, k3, k4, k5 = st.columns(5)
k1.metric("Fatturato 2030", eur(float(ricavi[-1])))
k2.metric("EBITDA 2030", eur(float(ebitda[-1])))
k3.metric("ROI semplice (orizzonte)", f"{roi_semplice*100:.0f}%" if np.isfinite(roi_semplice) else "n/a")
...
# ------------------------------------------------------------
# CRUSCOTTO COERENZA SICILIA (Palermo nel contesto regionale)
# ------------------------------------------------------------
st.subheader("📌 Cruscotto coerenza Sicilia — Palermo nel contesto (dati chiari)")

st.caption(
    "Questa vista serve a *contestualizzare* Palermo: quali province crescono di più, "
    "quali hanno il maggior stock BEV e che forecast 2030 ne deriva (metodo esplicito)."
)

# Tabella riepilogo per tutte le province
prov_list = sorted(df_bev["Provincia"].unique())
rows = []
sicily_2024_total = df_bev[df_bev["Anno"] == 2024]["Elettrico"].sum()

for prov in prov_list:
    subp = df_bev[df_bev["Provincia"] == prov].sort_values("Anno")
    bev_2024_p = int(subp[subp["Anno"] == 2024]["Elettrico"].iloc[0]) if (subp["Anno"] == 2024).any() else np.nan

    # CAGR 2021-2024 (se disponibile) + fallback 2015-2024 (se disponibile)
    def cagr(from_year, to_year):
        if (subp["Anno"] == from_year).any() and (subp["Anno"] == to_year).any():
            v0 = float(subp[subp["Anno"] == from_year]["Elettrico"].iloc[0])
            v1 = float(subp[subp["Anno"] == to_year]["Elettrico"].iloc[0])
            if v0 > 0 and v1 > 0:
                return (v1 / v0) ** (1 / (to_year - from_year)) - 1
        return np.nan

    cagr_21_24 = cagr(2021, 2024)
    cagr_15_24 = cagr(2015, 2024)

    # Forecast 2030 con lo stesso metodo selezionato in sidebar
    _, bev_2030_p, cagr_used_p = forecast_bev_2030(df_bev, prov, bev_forecast_method)

    share_2024 = (bev_2024_p / sicily_2024_total) if (sicily_2024_total > 0 and np.isfinite(bev_2024_p)) else np.nan

    rows.append({
        "Provincia": prov,
        "BEV 2024": bev_2024_p,
        "Quota Sicilia 2024": share_2024,
        "CAGR 21–24": cagr_21_24,
        "CAGR 15–24": cagr_15_24,
        f"Forecast 2030 ({bev_forecast_method})": int(bev_2030_p) if np.isfinite(bev_2030_p) else np.nan,
        "CAGR usato (forecast)": cagr_used_p
    })

df_region = pd.DataFrame(rows).sort_values("BEV 2024", ascending=False)

cA, cB = st.columns([1.2, 1])

with cA:
    st.write("**A) Ranking province + crescita (tabella)**")
    st.dataframe(
        df_region.style.format({
            "Quota Sicilia 2024": "{:.1%}",
            "CAGR 21–24": "{:.1%}",
            "CAGR 15–24": "{:.1%}",
            "CAGR usato (forecast)": "{:.1%}",
        })
    )
    st.caption(f"Dataset: {DATASET_NAME} — versione: {DATASET_VERSION}")

with cB:
    st.write("**B) Andamento BEV (tutte le province) + totale Sicilia**")
    pivot = df_bev.pivot_table(index="Anno", columns="Provincia", values="Elettrico", aggfunc="sum").sort_index()
    pivot["SICILIA (totale)"] = pivot.sum(axis=1)

    # Per leggibilità: plot solo Top N province (per BEV 2024) + Palermo + totale Sicilia
    topN = 5
    top_prov = df_region["Provincia"].head(topN).tolist()
    cols = list(dict.fromkeys(top_prov + ["PALERMO", "SICILIA (totale)"]))  # unique keep order
    cols = [c for c in cols if c in pivot.columns]

    fig, ax = plt.subplots()
    for c in cols:
        ax.plot(pivot.index, pivot[c], marker="o", linewidth=2, label=c)
    ax.set_xlabel("Anno")
    ax.set_ylabel("BEV (stock)")
    ax.legend()
    st.pyplot(fig)

    pal_2024 = int(df_region[df_region["Provincia"] == "PALERMO"]["BEV 2024"].iloc[0]) if (df_region["Provincia"] == "PALERMO").any() else np.nan
    st.markdown(
        f"""
**Lettura rapida**
- **Palermo 2024**: **{pal_2024:,}** BEV (quota Sicilia ~ **{(pal_2024/sicily_2024_total*100):.1f}%**).  
- Metodo forecast selezionato: **{bev_forecast_method}**.
        """.replace(",", ".")
    )
